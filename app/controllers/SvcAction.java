...
public class SvcAction extends play.mvc.Action.Simple {
	private final Logger.ALogger log = play.Logger.of("application.PostAction");

	private final HttpExecutionContext ec;

	@Singleton
	@Inject
	public SvcAction(HttpExecutionContext ec, ActorSystem actorSystem) {
		this.ec = ec;
	}
			...
			if (e != null) {
				if (e instanceof CompletionException) {
					...
					if (completionException instanceof FailsafeException) {
						log.error("Circuit breaker is open!", completionException);
						return Results.status(SERVICE_UNAVAILABLE, "Service has timed out");
					} else {
						log.error("Direct exception " + e.getMessage(), e);
